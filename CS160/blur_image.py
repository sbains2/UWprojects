...
def read_grid(file_path):
    ...
    file_grid = []
    for text_line in main_file:
        rows = []
        for pixels in text_line.split(','):
            rows.append(int(pixels))
        file_grid.append(rows)
    main_file.close
    return (file_grid)
...
def blur(pixel_grid):
    ...
    for i in range(len(pixel_grid)):
        ...
        for x in range(len(pixel_grid[0])):
            row_grid.append(average_of_surrounding(pixel_grid, i, x))
        blurred_grid.append(row_grid)
...
if len(sys.argv) != 2:
    ...
    print("    (c) appropriate syntax <input_file> only ONE argument")
    print("\n  Blurs the given input file and outputs the result as:")
